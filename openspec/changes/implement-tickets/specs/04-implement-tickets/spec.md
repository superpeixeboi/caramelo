...
The system SHALL allow creating tickets with `phoneNumber`, `flow`, `status`, `context`, `currentStepIndex`, and `steps` array.

#### Scenario: Create ticket
- **WHEN** `POST /tickets` is called with `{ phoneNumber: "5511999999999", flow: "service-request", status: "open", steps: [...], currentStepIndex: 0 }`
